# Challenge 6: CUME_DIST challenge 6: finish time inside each race event

## Task

Calculate the cumulative distribution of finish time inside each race event while keeping every detail row.

## Requirements

- Use `CUME_DIST()`.
- Partition by `EventName`.
- Order by `FinishSeconds`.

## Tables

- `dbo.RaceResults`

## Setup

Run these files first:

1. `00_Database_Setup/01_AllInOne_Create_Database.sql`
2. `00_Database_Setup/03_Extended_Challenge_Data.sql`

## Try first

Write your query in `MySol.sql` before opening `Solution.sql`.
