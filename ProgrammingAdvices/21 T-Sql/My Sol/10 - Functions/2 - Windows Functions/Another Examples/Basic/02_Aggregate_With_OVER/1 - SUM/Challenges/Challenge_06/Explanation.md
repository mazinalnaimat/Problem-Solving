...
- `SUM` is evaluated over a window, so detail rows are preserved.
- `PARTITION BY AthleteName` restarts the calculation for each logical group.
- The frame `ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING` controls which neighboring rows participate.
